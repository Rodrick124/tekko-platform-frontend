...
function Header() {
  ...
  return <header className="sticky top-0 z-50 border-b border-slate-200/80 bg-white/95 shadow-[0_1px_8px_rgba(15,23,42,.05)] backdrop-blur-md">
    <div className="mx-auto flex h-20 max-w-6xl items-center justify-between gap-4 px-4 lg:px-8">
      ...
      <div className="hidden items-center gap-2 sm:flex">{isAuthenticated ? <><NavLink to={ROUTES.client} className="inline-flex items-center gap-2 rounded-lg px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-100"><UserRound size={16}/><span className="max-w-28 truncate">{user.fullName}</span></NavLink><button type="button" onClick={logout} className="inline-flex items-center gap-2 rounded-lg bg-blue-600 px-4 py-2 text-sm font-semibold text-white shadow-sm hover:bg-blue-700 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-600 focus-visible:ring-offset-2"><LogOut size={16}/>Déconnexion</button></> : <><NavLink to={ROUTES.login} className="rounded-lg px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-100">Se connecter</NavLink><NavLink to={ROUTES.register} className="rounded-lg bg-blue-600 px-4 py-2 text-sm font-semibold text-white shadow-sm hover:bg-blue-700">Créer un compte</NavLink><span className="grid size-8 place-items-center rounded-full bg-blue-700 text-white"><UserRound size={16}/></span></>}</div>
    ...
    {open && <nav id="mobile-navigation" className="border-t bg-white px-4 py-4 lg:hidden" aria-label="Navigation mobile"><div className="mx-auto grid max-w-6xl gap-1">{navigation.map((item) => <NavLink key={item.to} to={item.to} end={item.end} className={({ isActive }) => `rounded-lg px-4 py-3 font-semibold ${isActive ? 'bg-blue-50 text-blue-700' : 'text-slate-700 hover:bg-slate-50'}`} onClick={() => setOpen(false)}>{item.label}</NavLink>)}{isAuthenticated ? <><NavLink to={ROUTES.client} className="rounded-lg px-4 py-3 font-semibold text-slate-700" onClick={() => setOpen(false)}>Mon espace — {user.fullName}</NavLink><button type="button" className="mt-2 inline-flex items-center justify-center gap-2 rounded-lg bg-blue-600 px-4 py-3 font-semibold text-white" onClick={() => { logout(); setOpen(false) }}><LogOut size={17}/>Se déconnecter</button></> : <><NavLink to={ROUTES.login} className="rounded-lg px-4 py-3 font-semibold text-slate-700" onClick={() => setOpen(false)}>Se connecter</NavLink><NavLink to={ROUTES.register} className="mt-2 rounded-lg bg-blue-600 px-4 py-3 text-center font-semibold text-white" onClick={() => setOpen(false)}>Créer un compte</NavLink></>}</div></nav>}
  </header>
}
